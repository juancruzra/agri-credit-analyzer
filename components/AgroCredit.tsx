 ...
 * Cambios claves:
 * - Repago por waterfall: primero paga costos NO financiados (labores+arrendamiento) y luego el crédito.
 *   Condición por simulación: R >= (RestCost) + (L + I)  ⇔  (R - (Inputs+RestCost)) >= (L - Inputs) + I
 *   Si L = InputsTotales ⇒ umbral = I (margen debe cubrir intereses).
 * - Máximo de crédito: HASTA INSUMOS (no más).
 * - Mantiene "año malo/típico/muy bueno" con márgenes (p5/p50/p95).
 * - Semáforo: Verde ≥85%, Amarillo [60%,85%), Rojo <60% (amarillo es un solo rango entre fin del verde e inicio del rojo).
 * - Resalta: “Crédito máximo que cumple objetivo”.
 */
...
export default function AgroCredit(){
  ...
  function run(){
    ...
    // NECESIDADES determinísticas por medias (para panel de "Necesidades y costos")
    ...
    // GRILLA de crédito: 0..INSUMOS (no más)
    ...
    for (let s = 0; s < GRID_STEPS; s++) {
      ...
      const L = needInputsUSD * pct; // tope: insumos
      const I = L * RATE;

      // Waterfall de repago: primero Rest (labores+rent), luego crédito
      // Condición: R >= Rest + (L + I)
      // Equivalente con margen: (R - (Inputs+Rest)) >= (L - Inputs) + I
      let repayCount = 0;
      for (let i=0;i<N;i++){
        // Usamos la forma directa con R y Rest (misma lógica)
    ...
    // RANGO VERDE (tramo continuo con prob ≥ objetivo)
    ...
    // AMARILLO: un solo rango entre fin del verde y comienzo del rojo, con 60% ≤ prob < 85%
    ...
    // ROJO: desde el primer índice rojo hasta el final
    ...
    // INSIGHTS (malo/típico/bueno + % positivo)
